...
static std::pair<std::vector<float>, std::vector<int>> generate_matrix(
    ...
    std::vector matrix(rows, std::vector<float>(n_features));
    ...
    for_each(std::execution::par_unseq, indices.begin(), indices.end(),
        [&](const long i) {
            ...
            for (long j = 0; j < n_features; ++j) {
                matrix[i][j] = feature_dist(gen);

                linear_combination += coefficients[j] * matrix[i][j];
            }
            ...
            if (n_features >= 4) {
                ...
                if (base_idx + 2 < n_features) {
                    matrix[i][n_features - 2] = 0.7f * matrix[i][base_idx] +
                                               0.3f * matrix[i][base_idx + 1] +
                                               noise_dist(gen);
                }
                if (n_features >= 3) {
                    matrix[i][n_features - 1] = 0.5f * matrix[i][0] + noise_dist(gen);
                }
            }
    ...
    return std::pair{flatten(matrix, rows * n_features), labels};
}
#endif //UTILS_H
